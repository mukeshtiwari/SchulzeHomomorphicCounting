...
let convert_fun_dec_ballot paircand lstbal =
...
let encrypt_zero_margin cand_all pubkey ballot = 
  let paircand = all_pairs cand_all in
  let ballotvalue = List.map (fun (c, d) -> Big.to_string (ballot c d)) paircand in (* convert the ballot into list of values and change it to string *)
  let zeroencmargin = Javaocamlbinding.enc_ballot ballotvalue (Big.to_string pubkey) in
  convert_fun_enc_ballot paircand zeroencmargin   
...
let decrypt_ballot_with_zkp cand_all prikey eballot =
  ...
  let decballot = Javaocamlbinding.dec_ballot (convert_eballot encballotvalue) (Big.to_string prikey) in 
...
let row_permute_encrypted_ballot cand_all pubkey eballot =
  ...
  let rowperm = Javaocamlbinding.row_perm (convert_eballot rowpermstr) (Big.to_string pubkey) in
...
let column_permute_encrypted_ballot cand_all pubkey eballot =
  ...
  let colperm = Javaocamlbinding.column_perm (convert_eballot colpermstr) (Big.to_string pubkey) in
...
let homomorphic_add_eballots cand_all emarg eballot =
  ...
  let finalstr = Javaocamlbinding.homomorphic_addition (convert_eballot margstr) (convert_eballot ebalstr) in
  convert_fun_enc_ballot paircand finalstr

type 'cand hCount =
| Ax of 'cand eballot list * ('cand -> 'cand -> ciphertext)
...
let all_ballots_counted cand_all0 dec_cand bs =
  let enczmargin =
    encrypt_zero_margin cand_all0 publickey (fun _ _ -> Big.zero)
  in
  ...
  let h = Ax (bs, enczmargin, decmarg, ezkp) in x h
